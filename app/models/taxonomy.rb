class Taxonomy
  ...
  def create_tree_item(items, key = I18n.t('mongoid.models.taxonomy.node_key'))
    node = {
        'amount' => items[:amount],
        'key' => key,
  ...
  def self.load_from_csv file_name
    ...
    CSV.foreach(file_name, {:headers => true, :col_sep => ";"}) do |row|
      items[row[0]] = { title: row[I18n.t('mongoid.models.taxonomy.short_title')], color: row[I18n.t('mongoid.models.taxonomy.color')], icon: row[I18n.t('mongoid.models.taxonomy.icon')], description: row[I18n.t('mongoid.models.taxonomy.description')] }
    end
    items
  end
