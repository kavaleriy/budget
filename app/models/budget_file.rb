class BudgetFile
  ...
  def load_file
    ...
    raw.first_column.upto(raw.last_column - 1) do |col|
      self.taxonomies << { :key => "taxonomy#{col}", :title => raw.cell(1, col) }
    end
    ...
    2.upto(raw.last_row) do |line|
      ...
      raw.first_column.upto(raw.last_column - 1) do |col|
        row["taxonomy#{col}"] = raw.cell(line,col)
      end
      self.rows << row
    end
  end
  ...
  def prepare
    ...
    self.rows.each do |row|
      ...
      row.keys.reject{|key| key == :amount}.each do |key|
        # self.tree_info[key] = { 'title' => '' }
      end
    end

    self.meta_data = { :min => min, :max => max}

    self.tree = create_bubble_tree
  ...
  def create_bubble_tree
    ...
    self.rows.each do |row|
      ...
      self.taxonomies.each { |taxonomy|
        ...
        curr_taxonomy_value = row[taxonomy[:key]]
        node[curr_taxonomy_value] = { :amount => row[:amount] } if node[curr_taxonomy_value].nil?
        node = node[curr_taxonomy_value]
      }
    end
