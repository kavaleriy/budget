class Revenue < BudgetFile
  ...
  def get_sunburst_tree(item = self.bubble_tree)
    sunburst_tree
  end

  def get_bubble_tree(item = self.bubble_tree)
    node = {
        'amount' => item[:amount],
        'key' => item[:key]
    }

    info = tree_info[item[:key]]
    if info
    ...
    unless item[:children].nil?
      ...
      item[:children].each { |child_node|
        node['children'] << self.get_bubble_tree(child_node)
      }
    end
  ...
  def load_file
    ...
    dbf.reject { |rec| rec.summ.nil? }.each do |rec|
      ...
      rows << {kkd: kkd, amount: amount}
    end

    self.bubble_tree = create_bubble_tree rows
    self.sunburst_tree = create_sunburst_tree rows
  end

  protected
    def create_bubble_tree rows
      kkd_codes = get_revenue_codes
      items = { :amount => 0 }

      self.tree_info = { 'Всього доходів' => { :color => 'green' } }
      ...
      rows.each do |r|
        if r[:amount] > 0
          ...
          self.tree_info[va] = { 'title' => kkd_codes[va.ljust(8, '0')] }
          self.tree_info[vb] = { 'title' => kkd_codes[vb.ljust(8, '0')] }
          self.tree_info[vc] = { 'title' => kkd_codes[vc.ljust(8, '0')] }
          self.tree_info[vd] = { 'title' => kkd_codes[vd.ljust(8, '0')] }
        end
      end

      create_bubble_tree_item(items)
    end

  def create_bubble_tree_item(items, key = 'Всього доходів')
    node = {
        'amount' => items[:amount],
        'key' => key,
    }
